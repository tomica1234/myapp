...
# Google Sheetを開く
sheet = client.open("app").worksheet("shopping")

# Google SheetをPandas DataFrameに読み込む
def load_data():
    df= pd.DataFrame(sheet.get_all_values(),columns = ['shopping_list'])
    data = df[1:]
...
for idx, row in data.iterrows():
    st.session_state.checked_state[idx] = st.checkbox(
        row['shopping_list'], key=f'checkbox_{idx}', value=st.session_state.checked_state[idx])
...
with st.form("add_data_form"):
    new_data = [st.text_input('shopping_list')]
