...
now_today = f'{str(now_y)}/{str(now_m)}/{str(now_d)}'

# '日付'列を日付型に変換
data['日付'] = pd.to_datetime(data['日付'])
# 年と月の情報を抽出して整数型に変換
data['年'] = data['日付'].dt.strftime('%Y').astype(int)  # 年をYYYY形式で抽出し、整数型に変換
data['月'] = data['日付'].dt.strftime('%m').astype(int)  # 月をMM形式で抽出し、整数型に変換
...
with st.form("add_data_form"):
    ...
    with col1:
        selected_place = st.selectbox("場所", store,index=0)
    with col2:
    ...
    if submit_add:
        sheet.append_row([now_today,selected_place,price])
...
st.write(fig)
st.dataframe(filtered_data.iloc[:,:4])